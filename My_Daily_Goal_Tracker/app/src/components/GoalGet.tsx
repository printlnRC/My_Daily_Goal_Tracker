    ...
    <div className="bg-base-200 p-6 rounded-2xl shadow-lg flex flex-col gap-4 h-full overflow-y-auto pr-10">
      ...
      {goals.map((goal) => {
        ...
        return (
          <div
            key={goal.id}
            className={`card bg-base-100 shadow-md border border-base-300 hover:shadow-lg transition-shadow relative cursor-pointer transition-all duration-200 ${isOpen ? "mb-28" : ""}`}
            style={{ marginBottom: isOpen ? "7rem" : undefined }}
          ...
          >
            ...
            >
              <div className="flex flex-row justify-between items-center gap-2 p-2 w-full bg-base-300 rounded-2xl shadow-md">
                ...
                <button
                  className="flex-1 relative overflow-hidden border border-red-600 text-red-500 font-medium bg-transparent px-4 py-2 rounded-xl transition-all duration-300 ease-in-out hover:text-white hover:bg-red-600 active:scale-95 whitespace-nowrap"
                  onClick={(e) => {
                    e.stopPropagation();
                    onDelete(goal.id);
                  }}
